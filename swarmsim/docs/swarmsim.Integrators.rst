...
where :math:`\mathbf{X}_t` represents the agent state vector, :math:`\mathbf{f}` is the drift term capturing deterministic dynamics, :math:`\mathbf{g}` is the diffusion term modeling stochastic fluctuations, and :math:`d\mathbf{W}_t` is a Wiener process increment.
...
.. automodule:: swarmsim.Integrators
   :members:
   :undoc-members:
   :show-inheritance:
...
* :doc:`swarmsim.Populations` - Agent population management
* :doc:`swarmsim.Interactions` - Force computation methods  
* :doc:`swarmsim.Simulators` - High-level simulation control
* :doc:`examples.basic_simulation` - Integration usage examples
